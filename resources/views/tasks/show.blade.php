...
@section('content')
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 dark:border-gray-600">
            <tbody>
            <tr class="border-b">
                <th class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-left w-1/4">{{ __('Name') }}</th>
                <td class="px-6 py-4">{{ $task->name }}</td>
            </tr>
            <tr class="border-b">
                <th class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-left">{{ __('Status') }}</th>
                <td class="px-6 py-4">{{ $task->status->name ?? __('Without a status') }}</td>
            </tr>
            <tr class="border-b">
                <th class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-left">{{ __('Creator') }}</th>
                <td class="px-6 py-4">{{ $task->creator->name }}</td>
            </tr>
            <tr class="border-b">
                <th class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-left">{{ __('Executor') }}</th>
                <td class="px-6 py-4">{{ $task->assignee->name ?? __('Not assigned') }}</td>
            </tr>
            <tr class="border-b">
                <th class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-left">{{ __('Create date') }}</th>
                <td class="px-6 py-4">{{ $task->created_at->format('d.m.Y H:i') }}</td>
            </tr>
            <tr class="border-b">
                <th class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-left">{{ __('Description') }}</th>
                <td class="px-6 py-4">{{ $task->description ?? __('Without a description') }}</td>
            </tr>
            <tr>
                <th class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-left">{{ __('Labels') }}</th>
                <td class="px-6 py-4">
